...
CFG="$CFG" ANCHORS="$ANCHORS" HOOK="$HOOK" python3 - <<'PY'
import json, os
...
# .claude/settings.json — enable + creds + the recall hook (points at the GLOBAL script).
...
# .mcp.json — the hivemind MCP server, scoped to this project (so tools appear only here).
m = load(".mcp.json")
...
save(".mcp.json", m)
print("HiveMind AAN voor dit project: .claude/settings.json (hook + env) + .mcp.json (MCP ->", mcp_url + ")")
PY
