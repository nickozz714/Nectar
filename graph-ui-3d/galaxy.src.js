/* NECTAR // MIND — GALAXY: 3D drill-down in twee niveaus.
   Niveau 1 toont alléén de topics als sterren; klik op een ster en je reist naar
   dat stelsel: het topic als zon met z'n kenniskinderen eromheen. RELATES-links
   naar buiten worden korte "portaal"-stompjes waarmee je doorspringt. */
...
function makeNode(n) {
  ...
  const r = role === "sun" ? 8.5 : role === "star" ? starR(n) : role === "portal" ? 1.0 : kidR(n);
  ...
  const size = role === "sun" ? 4.2 : role === "star" ? 5.6 : role === "portal" ? 1.8 : 2.2;
...
const Graph = new ForceGraph3D(el("graph"), { controlType: "orbit" })
  ...
  .onNodeClick(n => {
    ...
    if (state.level === 1) { enterSystem(n.id); return; }
    if (n.__role === "portal") {
...
function tuneForces(level) {
  Graph.d3Force("charge").strength(level === 1 ? -190 : -80);
  Graph.d3Force("link").distance(l => ({ tt: 110, sun: 62, rel: 48, portal: 30 }[l.__kind] || 60));
  /* zachte trek naar het centrum: losse sterren blijven deel van ÉÉN galaxy */
  Graph.d3Force("cohere", (() => {
    const k = level === 1 ? 0.03 : 0.015;
    let nodes = [];
    const f = alpha => {
...
function backToGalaxy() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => Graph.zoomToFit(900, 70), 500);
...
function updateHud() {
  if (state.level === 1) {
    ...
    el("crumbPath").innerHTML = `GALAXY <span class="sep">▸</span> <span class="here">${esc(titleOf(topic))}</span>`;
