...
/* ── volledige node-acties in het cockpit-paneel (pariteit met legacy/mind) ── */
function buildExtras(id, f) {
  ...
  const refreshAll = async () => { detailCache.delete(id); await loadData(); render(false); renderCrumbs(); renderPanel(); };
  ...
  box.innerHTML = `
    ...
    <div class="sec"><h4 title="artefacten centraal in de hive — klik een naam voor preview">📎 bijlagen</h4>
    ...
    <div class="sec"><h4>acties</h4>
      <span class="mini" id="xLineage" title="herkomst + governance-events">🧬 lineage</span>
      ${!isTopic ? `<span class="mini" id="xSuggest" title="wijziging voorstellen — consensus beslist">✎ voorstel</span>
      <span class="mini" id="xArchive" title="archiveren voorstellen">🗑 archiveer</span>` : ""}
    ...
    <div class="sec" style="color:var(--dim);font-size:10.5px">door ${esc2(f.created_by_model || "onbekend model")}${f.sensitivity === "gevoelig" ? " · 🔒 gevoelig" : ""}</div>`;

  const $$ = i2 => box.querySelector("#" + i2);
  const on = (i2, fn) => { const x = $$(i2); if (x) x.onclick = () => fn().then(refreshAll).catch(e => alert(e.message)); };
  ...
  if (mrg) mrg.onclick = async () => {
    ...
    try { await capi("/graph/topics/merge", { method: "POST", body: JSON.stringify({ from_topic: f.title, into_topic: into }) }); refreshAll(); }
    catch (e) { alert(e.message); }
  };
  const sup = $$("xSup");
  if (sup) {
    let chosen = null, tmr2;
    sup.oninput = () => { clearTimeout(tmr2); tmr2 = setTimeout(async () => {
      const q2 = sup.value.trim(); const pick = $$("xSupPick");
      if (q2.length < 2) { pick.style.display = "none"; return; }
      const rs = (await capi(`/graph/search?q=${encodeURIComponent(q2)}`)).slice(0, 8);
      pick.innerHTML = rs.map((r2, i3) => `<div data-i="${i3}">${esc2(r2.title)} <span style="color:var(--dim)">· ${esc2(r2.type)}</span></div>`).join("");
      pick.style.display = "block";
      pick.querySelectorAll("[data-i]").forEach(dv => dv.onclick = () => {
        chosen = rs[+dv.dataset.i]; sup.value = chosen.title; pick.style.display = "none";
        $$("xSupSel").textContent = "✓ " + chosen.title; });
    }, 260); };
    $$("xSupSave").onclick = async () => {
      if (!chosen) { alert("Zoek en kies eerst de nieuwere node"); return; }
      try { await capi("/graph/supersede", { method: "POST", body: JSON.stringify({ old_uid: id, new_uid: chosen.uid }) }); refreshAll(); }
      catch (e) { alert(e.message); }
    };
  }
  const lin = $$("xLineage");
  if (lin) lin.onclick = async () => {
    ...
    try {
      ...
      o.innerHTML = [["persoon", L.created_by_person], ["account", L.created_by_account], ["model", L.created_by_model],
        ["gevoeligheid", L.sensitivity], ["aangemaakt", L.created_at ? new Date(L.created_at).toLocaleString("nl-NL") : null]]
        .filter(([, v]) => v).map(([k, v]) => `<div style="margin-top:3px"><span style="color:var(--dim)">${k}:</span> ${esc2(String(v))}</div>`).join("") +
        ((L.events || L.audit || []).slice(0, 15).map(ev => `<div style="color:var(--dim);margin-top:2px">▸ ${esc2(ev.action || "?")} · ${esc2(ev.account || "")}</div>`).join(""));
    } catch { o.innerHTML = `<div style="color:#ff7847">lineage niet op te halen</div>`; }
  };
  const sug = $$("xSuggest");
  if (sug) sug.onclick = async () => {
    const content = prompt("Voorgestelde nieuwe inhoud (consensus beslist):");
    if (!content) return;
    try { await capi("/graph/suggest", { method: "POST", body: JSON.stringify({ kind: "edit", node_uid: id, payload: { content }, rationale: prompt("Waarom?") || "via cockpit", model_name: "mens-via-cockpit" }) });
      alert("✓ voorstel ingediend — zie de Pollinate-deck"); } catch (e) { alert(e.message); }
  };
  const arch = $$("xArchive");
  if (arch) arch.onclick = async () => {
    const reason = prompt("Archiveren voorstellen — reden:");
    if (!reason) return;
    try { await capi("/graph/suggest", { method: "POST", body: JSON.stringify({ kind: "invalidate", node_uid: id, payload: { reason }, rationale: reason, model_name: "mens-via-cockpit" }) });
      alert("✓ archiveer-voorstel ingediend"); } catch (e) { alert(e.message); }
  };
  const del = $$("xDelete");
  if (del) del.onclick = async () => {
    if (!confirm(`Permanent verwijderen?\n\n"${n.title}"\n\nDit kan niet ongedaan worden gemaakt.`)) return;
    try { await capi(`/graph/node/${id}`, { method: "DELETE" }); goBack(); refreshAll(); } catch (e) { alert(e.message); }
  };

  capi(`/graph/node/${id}/attachments`).then(list => {
    const ab = $$("xAtts"); if (!ab) return;
    ab.innerHTML = (list || []).map(a => `<div style="margin:5px 0">
        <div style="display:flex;gap:8px;align-items:center">
          <a href="#" class="attname" data-prev="${esc2(a.uid)}" data-fn="${esc2(a.filename)}">📎 ${esc2(a.filename)}</a>
          ...
          ${maintain ? `<span class="mini" data-del2="${esc2(a.uid)}" style="color:#ff7847" title="verwijderen">×</span>` : ""}
        </div><div id="xprev-${esc2(a.uid)}"></div></div>`).join("") || "geen bijlagen";
    const getBlob = async uid2 => { const r2 = await fetch(`/attachments/${uid2}`, AUTH); if (!r2.ok) throw new Error("niet op te halen"); return r2; };
    ab.querySelectorAll("[data-dl]").forEach(b2 => b2.onclick = async () => {
      try { const r2 = await getBlob(b2.dataset.dl); const a3 = document.createElement("a");
        a3.href = URL.createObjectURL(await r2.blob()); a3.download = b2.dataset.fn; a3.click(); } catch (e) { alert(e.message); } });
    ab.querySelectorAll("[data-del2]").forEach(b2 => b2.onclick = async () => {
      if (!confirm("Bijlage verwijderen?")) return;
      try { await capi(`/attachments/${b2.dataset.del2}`, { method: "DELETE" }); detailCache.delete(id); renderPanel(); } catch (e) { alert(e.message); } });
    ab.querySelectorAll("[data-prev]").forEach(b2 => b2.onclick = async ev => {
      ev.preventDefault();
      const pv = ab.querySelector(`[id="xprev-${b2.dataset.prev}"]`);
      if (pv.innerHTML) { pv.innerHTML = ""; return; }
      ...
      try {
        ...
        const blob = await r2.blob(); const fn = b2.dataset.fn;
        ...
        const meta = `<div style="color:var(--dim);font-size:10px;margin:4px 0">${esc2(ct || "onbekend")} · ${(blob.size / 1024).toFixed(1)} kB</div>`;
        if (ct.startsWith("image/")) pv.innerHTML = meta + `<img src="${URL.createObjectURL(blob)}" style="max-width:100%">`;
        else if (textish) { const t2 = await blob.text();
          pv.innerHTML = meta + `<pre style="max-height:240px;overflow:auto;background:rgba(62,224,255,.05);border:1px solid var(--line);padding:8px;font-size:10.5px;white-space:pre-wrap;margin:0">${esc2(t2.slice(0, 20000))}</pre>`; }
  ...
  }).catch(() => { const ab = $$("xAtts"); if (ab) ab.textContent = ""; });
  const up = $$("xAttUp");
  if (up) up.onclick = async () => {
    ...
    if (!r2.ok) { alert("upload mislukt"); return; }
    detailCache.delete(id); renderPanel();
  };
}
...
function initSearch() {
  ...
  input.addEventListener("input", () => {
    ...
    if (!SERVER) return;
    clearTimeout(semTmr);
    semTmr = setTimeout(async () => {                       // semantische hive-zoek mengt bij
      try {
...
async function main() {
  ...
  initEmbed();
  if (SERVER) {
    capi("/graph/me").then(m => { MEg = m; }).catch(() => {});
